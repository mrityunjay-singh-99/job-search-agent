...
from config import GEMINI_API_KEY, LLM_MODEL, JOB_PLATFORMS
...
class JobSearchAgent:
    ...
    def search_jobs(self, resume_data, keywords, location, platforms=None, count=5):
        ...
        # Try the SerpAPI approach first (then we give links)
        api_jobs = []
    ...
    def get_job_match_analysis(self, resume_data, job_data):
        ...
        try:
            ...
            # Create a template for matching analysis
            template = """
            ...
            input_values = {
                ...
                "experience": "\n".join([f"- {exp}" for exp in resume_data.get("experience", [])]),
                "job_title": job_data.get("title", ""),
                "job_description": job_data.get("description", "")
            }
    ...
    def _generate_basic_match_analysis(self, resume_data, job_data):
        """Generate basic job match analysis when GeminiAI is not available."""
        skills = resume_data.get("skills", [])
